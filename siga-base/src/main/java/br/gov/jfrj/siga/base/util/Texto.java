...
public class Texto {
    ...
    public static String extrai(final String sSource, final String sBegin, final String sEnd) {
        final int iBegin = sSource.indexOf(sBegin);
        final int iEnd = sSource.indexOf(sEnd);

        if (iBegin == -1 || iEnd == -1)
            return null;

        return sSource.substring(iBegin + sBegin.length(), iEnd);
    }
    ...
    public static String pluralizar(String s) {
        ...
        for (String palavra : s.trim().split("\\s")) {
            ...
            pluralFinal.append(pluralFinal.length() > 0 ? " " : "")
                    .append(palavraPlural);
        }
