...
public class PermissaoUsuarioRelatorio extends RelatorioTemplate {
    ...
    public PermissaoUsuarioRelatorio(Map parametros) throws DJBuilderException {
        ...
        long idPessoa;
        try {
            idPessoa = Long.parseLong((String) parametros.get("idPessoa"));
        ...
        parametros.put("brasao", obterUrlBrasao(dpPessoa.getOrgaoUsuario().getId()));
    }
    ...
    private void processarConfiguracaoAcesso(ConfiguracaoAcesso cfga, List<String> dados) {
        try {
            dados.add(printSeparadorNivel(cfga.getServico().getNivelHierarquico()) + printServico(cfga.getServico()));
        } catch (Exception e) {
            dados.add("");
        }
        try {
        ...
        try {
            dados.add(String.valueOf(cfga.getCadastrante().getSesbPessoa() + cfga.getCadastrante().getMatricula()));
        } catch (Exception e) {
            dados.add("");
        }
    }
