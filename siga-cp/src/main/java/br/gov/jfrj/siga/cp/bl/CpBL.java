...
public class CpBL {
        ...
        if (id != null) {
            ...
            if (pessoaAnt != null) {
                ...
                if (podeAlterarOrgaoPessoa && !idLotacao.equals(pessoaAnt.getLotacao().getId())) {
                    ...
                    if (qtdeCaixaEntradaTMPPessoa > 0 || (qtdeCaixaEntradaTMPLotacao > 0 && qtdePessoaLotacao.equals(1L)))
                        throw new AplicacaoException(
    ...
    public Boolean validaHashPin(String pin, CpIdentidade identidadeCadastrante) throws
            RegraNegocioException, NoSuchAlgorithmException {
    ...
    public Boolean validaPinIdentidade(String pin, CpIdentidade identidadeCadastrante) throws
            RegraNegocioException, NoSuchAlgorithmException {
    ...
    public Boolean isTokenValido(Long tipoToken, Long cpf, String token) {
        ...
        try {
            CpToken tokenResetPin = dao().obterCpTokenPorTipoToken(2L, token);
    ...
    private String textoEmailDefinicaoPin(DpPessoa destinatario, String corpo) {
        ...
        try (BufferedReader bfr = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("/templates/email/novo-pin-definido.html"), StandardCharsets.UTF_8))) {
            ...
            conteudo = conteudo
                    .replace("${url}", Prop.get("/siga.internal.base.url"))
    ...
    public DpLotacao criarLotacao(final CpIdentidade identidadeCadastrante, final DpPessoa titular,
    ...
    public DpCargo gravarCargo(final CpIdentidade identidadeCadastrante, final Long id, final String nmCargo,
                               final Long idOrgaoUsu, final Boolean isAtivo) throws
    ...
    public DpFuncaoConfianca gravarFuncaoConfianca(final CpIdentidade identidadeCadastrante, final Long id,
                                                   final String nmFuncaoConfianca, final Long idOrgaoUsu, final Boolean isAtivo) throws
            IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        if (identidadeCadastrante == null) {
            throw new AplicacaoException("Não é possível cadastrar ou alterar o cargo sem que seja informado o cadastrante.");
        }
