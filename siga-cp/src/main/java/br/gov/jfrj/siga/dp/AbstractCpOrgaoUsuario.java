...
public abstract class AbstractCpOrgaoUsuario extends Objeto implements
    ...
    @JoinColumn(name = "ID_BLOB_BRASAO")
    private SigaBlob brasao;
    @Transient  private SigaBlob oldBrasao;
    ...
    public SigaBlob getBrasao() {
        return brasao;
    }

    public void setBrasao(SigaBlob brasao) {
        this.oldBrasao = this.brasao;
        this.brasao = brasao;
    }

    public byte[] getBrasaoBytes() {
        if (brasao == null)
            return null;

        return brasao.getData();
    }

    public void setBrasaoBytes(byte[] data) {
        if (data == null)
            return;
    ...
    private void postUpdate() {
        ...
        if (brasao == null && oldBrasao != null) {
            ContextoPersistencia.em().remove(oldBrasao);
        }
    }
