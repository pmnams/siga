...
public class AcessoServicoRelatorio extends RelatorioTemplate {
    ...
    public AcessoServicoRelatorio(Map parametros) throws DJBuilderException {
        ...
        try {
            if (parametros.get("idOrgaoUsuario").equals("-1")) {
                ...
                if (!cpOrgaosUsuario.isEmpty()) {
                    idOrg = cpOrgaosUsuario.get(0).getIdOrgaoUsu();
                }
            } else {
        ...
        parametros.put("brasao", obterUrlBrasao(idOrg));
    }
     ...
     *
     * @param arl ArrayList com as configurações a rodenar
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * @param cpSituacoesConfiguracao the cpSituacoesConfiguracao to set
     */
