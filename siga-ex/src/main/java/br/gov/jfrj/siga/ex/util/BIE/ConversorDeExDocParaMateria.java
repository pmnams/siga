...
public class ConversorDeExDocParaMateria {
    ...
    public List<Materia> converter(List<ExDocumento> docs) {
        ...
        for (ExDocumento doc : docs) {
            ...
            String siglaFormaDocAnexo = "ANE";
            for (ExDocumento filho : doc.getExDocumentoFilhoSet())
                if (!filho.isPendenteDeAssinatura() && filho.getExFormaDocumento().getSiglaFormaDoc().equals(siglaFormaDocAnexo)) {
    ...
    public Materia converter(ExDocumento doc, int cont) {
        ...
        if (m.getTipoMateria() == null || "".equals(m.getTipoMateria().trim()))
            m.setTipoMateria(
                    docBase.getExFormaDocumento().getSiglaFormaDoc()
                            + "|"
    ...
    private String enxugaHtml(String html) {
        ...
        while (posIni != -1) {
            ...
            html = html.substring(0, posIni)
                    + sub
                    + html.substring(posFim);

            int aPartirDe = posFim + 1;
