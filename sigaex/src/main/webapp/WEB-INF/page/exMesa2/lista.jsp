    ...
    <div id="app" class="container-fluid content">
        <div id="configMenu" class="mesa-config ml-auto" :class="toggleConfig">
            <c:if test="${!ehPublicoExterno}">
                <button type="button" class="btn-mesa-config btn btn-secondary btn-sm h-100"
            ...
            <div class="form-config border-bottom p-2">
                ...
                <div class="form-group my-1 border-bottom">
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="trazerComposto">
                            <small>Exibir indicador de doc. avulso <i class="far fa-file"></i> ou composto <i
                                    class="far fa-copy"></i></small>
                        </label>
                    </div>
                </div>
                <div class="form-group my-1 border-bottom">
                ...
                <div class="form-group my-1 border-bottom">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="trazerArquivados" v-model="trazerArquivados"
                               :disabled="carregando">
                ...
                <div class="form-group my-1 border-bottom">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="ordemCrescenteData"
                               v-model="ordemCrescenteData" :disabled="carregando">
                ...
                <div class="form-group my-1 border-bottom">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="usuarioPosse" v-model="usuarioPosse"
                               :disabled="carregando">
                ...
                <div class="form-group my-1 border-bottom">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="dtDMA" v-model="dtDMA"
                               :disabled="carregando">
                ...
                <div class="form-group pb-2 mb-1 border-bottom">
                    ...
                    <select name="selQtdPag" id="selQtdPag" class="form-control form-control-sm p-0" v-model="qtdPag" :class="{disabled: carregando}">
                ...
                <div class="form-group pt-2">
                    <button class="btn btn-secondary btn-sm py-0" @click="resetaStorage();"
                            :class="{disabled: carregando}">Limpar configurações
                    </button>
        ...
        <div aria-live="polite" aria-atomic="true" style="position: relative; min-height: 200px;">
            ...
            <div id="rowTopMesa" style="z-index: 1" class="row sticky-top px-3 pt-1 bg-white shadow-sm"
                ...
                <div class="input-group col-5 mb-1">
                    <input id="filtroExibidos" type="text" class="form-control p-1 input-sm"
                           @keyup.enter="recarregarMesa()" placeholder="Filtrar cód. ou descrição" v-model="filtro"
                           @keyup.enter="getItensGrupo();">
                    <div class="input-group-append">
                        <button class="btn btn-secondary border-light" type="button" @click="recarregarMesa();"><i
                    ...
                    <template v-for="g in grupos">
                        <div v-if="!g.hide && (g.grupoCounterUser > 0 || g.grupoCounterLota > 0)" :key="g.grupoOrdem">
                            <div class="collapse-header d-inline">
                                <h5 :id="['collapse-header-' + g.grupoOrdem]"
                                    :data-toggle="carregando ? '' : 'collapse'"
                                    :data-target="['#collapsetab-' + g.grupoOrdem]"
                                    class="collapse-toggle p-1 table-group table-group-title"
                                    :class="{ collapsed: g.grupoCollapsed}"
                                    v-bind:aria-expanded="{false: g.grupoCollapsed}"
                                    ...
                                    <small>
										<span class="badge badge-light btn-sm align-middle"
                                              :class="{disabled: exibeLota}">
                            ...
                            <div :id="['collapsetab-' + g.grupoOrdem]" class="collapse" :key="g.grupoOrdem"
                                 v-bind:class="{show: !g.grupoCollapsed }">
                                <div class="row"
                                     v-if="!carregando && (g.grupoDocs == undefined || g.grupoDocs.length == 0)">
                                    <div class="col col-12">
                                        <p class="alert alert-warning alert-dismissible fade show">Não há documentos a
                                            exibir.</p>
                                    </div>
                                </div>
                                <table v-else
                                       class="text-muted table table-sm table-striped table-hover table-borderless">
                                    ...
                                    <tr class="table-head d-flex">
                                        ...
                                        <th scope="col" class="d-none d-md-block"
                                            v-bind:class="usuarioPosse ? 'col-3' : 'col-4'">Descrição
                                        </th>
                                        <th scope="col" class="col-md-1"
                                            v-bind:class="usuarioPosse ? 'col-2' : 'col-3'">Origem
                                        </th>
                                        <th v-if="usuarioPosse" scope="col" class="col-md-1 col-2"><fmt:message
                                                key="documento.atendente"/></th>
                                        <th scope="col" class="col-3 d-none d-md-block"><fmt:message
                                                key="usuario.mesavirtual.etiquetas"/></th>
                                        <!-- 											<th v-show="gruposTemAlgumErro">Atenção</th> -->
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <template v-for="(f, index) in g.grupoDocs">
                                        <tr class="grupo-item d-flex mh-100"
                                            v-bind:class="f.codigo == ''? 'linha-ref':''" :data-grupo="g.grupoNome"
                                            :data-numitem="index">
                                            ...
                                            <td class="col-md-2"
                                                ...
                                                <c:if test="${siga_cliente == 'GOVSP'}">
														<span v-if="trazerComposto">
															<span v-if="f.tipoDoc == 'Avulso'"><i
                                                                    class="far fa-file text-secondary"
                                                                    title="Documento Avulso"></i></span>
															<span v-if="f.tipoDoc == 'Composto'"><i class="far fa-copy"
                                                                                                    title="Documento Composto"></i></span>
														</span>
                                                </c:if>
                                                <c:choose>
                                                ...
                                                <span class="d-inline d-md-none" v-if="f.descr != ''"
                                                      :title='processDescription(f.descr)'>- {{ processDescription(f.descr, 60)
                                                    }}
														</span>
                                                <span v-else><div v-if="f.datahora == '.'"
                                                                  class="spinner-dots my-2 mx-auto d-md-none"></div>
														<div v-else class="d-md-none">&nbsp;</div></span>
                                            </td>
                                            <td class="d-none d-md-block"
                                                v-bind:class="usuarioPosse ? 'col-3' : 'col-4'">
													<span class="text-break" v-if="f.descr != ''"
                                                          :title='processDescription(f.descr)'>{{ processDescription(f.descr, 60)
                                                        }}
														</span>
                                                <span v-else><div v-if="f.datahora == '.'"
                                                                  class="spinner-dots my-2 mx-auto"></div>
														<div v-else>&nbsp;</div></span>
                                            </td>
                                            <td class="col-md-1"
                                                ...
                                                <small>
                                                    <c:if test="${siga_cliente == 'GOVSP'}">
                                                        ...
                                                        <span v-if="f.dataDevolucao == 'alerta'"><i
                                                                class="fa fa-exclamation-triangle text-warning"></i></span>
                                                        <span v-if="f.dataDevolucao == 'atrasado'"><i
                                            ...
                                            <td v-if="usuarioPosse" class="col-md-1 col-2">
                                                <small>{{f.lotaPosse}}{{f.nomePessoaPosse ? ' / ' : ''}}
                                                    {{f.nomePessoaPosse}}</small>
                                            </td>
                                            <td class="col-3 d-none d-md-block p-1">
													<span v-if="f.anotacao != null">
														<a tabindex="0"
                                                           class="anotacao fas fa-sticky-note text-warning popover-dismiss ml-2"
                                                           role="button"
                                                           data-toggle="popover" data-trigger="hover focus"
                                                           :data-content="f.anotacao"></a>
													</span>
                                                <span class="xrp-label-container">
														<span v-for="m in f.list">
															<a class="popover-dismiss" role="button"
                                                               data-toggle="popover" data-trigger="hover focus"
                                                               :title="m.titulo" data-html="true"
                                                               :data-pessoa="m.pessoa" :data-lotacao="m.lotacao">
																<span class="text-size-8 badge badge-pill badge-secondary tagmesa m-1 btn-xs"
                                                                      v-bind:style="{color: '#' + m.cor + ' !important'}">
																	<i :class="m.icone"
                                                                       :style="{color: m.cor}"></i> {{m.nome}}<span
                                                                        v-if="m.ref1">, planejada: {{dtDMA ? m.ref1DDMMYYYY : m.ref1}}</span><span
                                                                        v-if="m.ref2">, limite: {{dtDMA ? m.ref2DDMMYYYY : m.ref2}}</span>
																</span>
															</a>
														</span>
													</span>
