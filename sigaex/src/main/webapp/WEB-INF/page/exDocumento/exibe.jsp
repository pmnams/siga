...
<siga:pagina titulo=" ${docVO.sigla}" popup="${param.popup}">
    ...
    <script type="text/ecmascript">
        if (${not empty f:resource('/vizservice.url')}) {
            ...
            document
                .write("<script src='/siga/javascript/viz.js' type='text/javascript'>"
                    + "<" + "/script>");
        }
            ...
            <div class="col">
                ...
                <h2>
                    ...
                    <button type="button" name="voltar"
                            onclick="${(empty param.linkVolta) ? 'javascript:window.location.href=\'/siga\';' : 'javascript:'.concat(param.linkVolta) }"
                            class="btn btn-secondary float-right ${hide_only_TRF2}" accesskey="r">Volta<u>r</u></button>
                </h2>
            </div>
        </div>
                ...
                <div>
                    ...
                    <c:choose>
                        ...
                        <c:when test="${docVO.doc.pdf}">
                            <c:set var="urlCapturado"
                                   value="/sigaex/app/arquivo/exibir?arquivo=${docVO.doc.referenciaPDF}"/>
                                    ...
                                    <c:if
							          ...
							          not mov.cancelada)}">
                                        <tr class="${mov.classe} ${mov.disabled}">
                                            <td class="text-left"
                                                title="${mov.dtRegMovDDMMYYHHMMSS}">${mov.tempoRelativo}</td>
                                            <td class="text-left"
                                            ...
                                            <td class="text-left"
                                                <c:if test="${mov.exTipoMovimentacao == 'ENCERRAMENTO_DE_VOLUME'}">data-toggle="tooltip"
                                                data-placement="top"
                                                title="O sistema encerra automaticamente um volume após a inclusão de ${f:resource('volume.max.paginas')} páginas para evitar lentidão no processamento e geração de PDF."
                                            </c:if>>
                                                    ${mov.descricao}
                                                ...
                                                <siga:links
                                                    ...
                                                    <c:forEach var="acao" items="${mov.acoes}">
                                                        <siga:link title="${acao.nomeNbsp}" pre="${acao.pre}"
                                                                   pos="${acao.pos}"
                                                                   url="${pageContext.request.contextPath}${acao.url}"
                                                                   test="${acao.pode}" explicacao="${acao.explicacao}"
                                                                   popup="${acao.popup}"
                ...
                <div class="gt-sidebar">
                                ...
                                <c:if test="${not empty m.anexosNaoAssinados}">
                                    ...
                                    <ul>
                                        <c:forEach var="naoAssinado" items="${m.anexosNaoAssinados}">
                                            <li><a
                                                    href="javascript:popitup('${pageContext.request.contextPath}/app/expediente/mov/exibir?id=${naoAssinado.idMov}&popup=true')"
                    ...
                    <c:if test="${not empty docVO.documentosPublicados or not empty docVO.boletim}">
                        <div class="card-sidebar card bg-light mb-3">
                            <tags:collapse title="Boletim Interno" id="BoletimInterno"
                    ...
                    <c:if test="${not empty docVO.outrosMobsLabel and not empty docVO.marcasDeSistemaPorMobil}">
                        <div class="card-sidebar card bg-light mb-3">
                            ...
                            <tags:collapse title="${docVO.outrosMobsLabel}" id="OutrosMob"
                                           collapseMode="${collapse_Expanded}" addToTitle="${butRefresh}"
                                           classInfo="m-0 p-0">
                                <div class="table-responsive">
									...
									<tr>
                                        ...
                                        <c:forEach var="entry" items="${docVO.marcasDeSistemaPorMobil}">
                                            ...
                                            <c:set var="mobNome"
                                                   value="${outroMob.isGeral() ? 'Geral' : outroMob.terminacaoSigla}"/>
                                                ...
                                                <tr class="${mov.classe} ${mov.disabled}">
                                                    <c:if test="${loop.first}">
                                                        <td rowspan="${entry.value.size()}"
                                                            style="padding-left: 1.25rem"><c:choose>
                                                    ...
                                                    <td><siga:selecionado isVraptor="true"
                                                                          sigla="${pessoaAtual.nomeAbreviado}"
                                                                          descricao="${pessoaAtual.descricao} - ${pessoaAtual.sigla}"
                                                                          pessoaParam="${pessoaAtual.siglaCompleta}"/></td>
                                                    <td><siga:selecionado isVraptor="true"
                                                                          sigla="${marca.dpLotacaoIni.lotacaoAtual.sigla}"
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${marca.exMovimentacao.podeCancelar(titular, lotaTitular)}">
                                                            <td style="padding-left:.25em; padding-right: 0">
                                                                <a href="javascript:postToUrl('/sigaex/app/expediente/mov/cancelar_movimentacao_gravar?id=${marca.exMovimentacao.idMov}&sigla=${sigla}')"
                                                                   title="${exibirExplicacao ? marca.exMovimentacao.expliquePodeCancelar(titular, lotaTitular) : ''}">
                    ...
                    <c:if test="${not empty docVO.marcasDoMobil}">
                        <div class="card-sidebar card bg-light mb-3">
                            <tags:collapse title="Marcadores" id="Marcadores" collapseMode="${collapse_Expanded}"
                                           classInfo="m-0 p-0">
                                <div class="table-responsive">
                                    <table class="table table-sm mb-0 w-100">
                                        <tbody>
                                        <c:forEach var="marca" items="${docVO.marcasDoMobil}" varStatus="loop">
                                            ...
                                            <c:if test="${loop.first or (marca.cpMarcador.idFinalidade.grupo != g)}">
                                                <tr>
                                                    <td colspan="6"
                                                        style="padding-left: 1.25rem; color: gray">${marca.cpMarcador.idFinalidade.grupo.nome}</td>
                                                </tr>
                                            ...
                                            <tr>
                                                <td style="padding-left: 2.5rem">${marca.descricaoComDatas}
                                                    <c:if test="${marca.cpMarcador.idFinalidade.idTpInteressado != 'ATENDENTE'}">
                                                        - <c:if test="${not empty pessoaAtual}"><siga:selecionado
                                                            isVraptor="true" sigla="${pessoaAtual.nomeAbreviado}"
                                                            descricao="${pessoaAtual.descricao} - ${pessoaAtual.sigla}"
                                                ...
                                                <c:choose>
                                                    <c:when test="${marca.exMovimentacao.podeCancelar(titular, lotaTitular)}">
                                                        <td style="padding-left:.25em; padding-right: 0">
                                                            <a href="javascript:postToUrl('/sigaex/app/expediente/mov/cancelar_movimentacao_gravar?id=${marca.exMovimentacao.idMov}&sigla=${marca.exMovimentacao.exMobil.sigla}&descrMov=' + encodeURIComponent('Exclusão de marcador: ${marca.cpMarcador.descrMarcador}'))"
                                                               title="${exibirExplicacao ? marca.exMovimentacao.expliquePodeCancelar(titular, lotaTitular) : ''}">
                                                                <i class="far fa-trash-alt"></i>
                                                            </a>
                                                        </td>
                                                    ...
                                                    <c:otherwise>
                                                        <td style="padding-left:.25em; padding-right: 0">
                                                            <a title="${exibirExplicacao ? marca.exMovimentacao.expliquePodeCancelar(titular, lotaTitular) : ''}">
                    ...
                    <div class="card-sidebar card bg-light mb-3">
                        ...
                        <tags:collapse
                                title="Documento"
                                id="DocDetalhes" collapseMode="${collapse_Expanded}">
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <c:if test="${not empty docVO.originalData}">-
                                            <b>original:</b> ${docVO.originalData}</c:if>
                                    </c:otherwise>
                                </c:choose>
                            ...
                            <c:if test="${not empty docVO.tipoDePrincipal and not empty docVO.principal}">
                                <p>
                                    <b>${docVO.tipoDePrincipal}:</b> <a
                                        href="/sigawf/app/procedimento/${docVO.principalCompacto}">${docVO.principal}</a>
                                </p>
                            </c:if>
                    ...
                    <c:if test="${not empty m.getDescricaoCompletaEMarcadoresEmHtml(cadastrante,lotaTitular)}">
                        <div class="card-sidebar card bg-light mb-3 ${hide_only_TRF2}">
                            <tags:collapse title="Situação do Documento" id="SituacaoDoc"
                                           collapseMode="${collapse_Expanded}">
                    ...
                    <c:if test="${not empty docVO.cossignatarios}">
                        <div class="card-sidebar card bg-light mb-3">
                            <tags:collapse title="Cossignatários" id="Cossignatários"
                                           collapseMode="${collapse_Expanded}">
                                <ul>
                    ...
                    <div class="card-sidebar card bg-light mb-3">
                        <tags:collapse title="Arquivos Auxiliares" id="ArqAuxiliares"
                                       collapseMode="${collapse_ArqAuxiliares}">
                                            ...
                                            <siga:links buttons="${false}" inline="${true}" separator="${false}">
                                                ...
                                                <div class="row ml-4 mb-3">
                                                    <small class="form-text text-muted mt-0">
                                                        <siga:link
                                                                title="${mov.mov.cadastrante.sigla}/${mov.mov.lotaCadastrante.sigla} - ${mov.tempoRelativo}"
                                                                test="${true}" classe="${acao.classe}"/>
                                                    </small>
                                                </div>
                                            </siga:links>
                                        </div>
    ...
    <c:if test="${recebimentoPendente  and !origemRedirectTransferirGravar}">
        ...
        <div class="modal fade" id="modalReceberDocumento" tabindex="-1" role="dialog" aria-labelledby="confirmacao"
             aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer text-center">
                        <div class="row" style="margin: 0 auto;">
                            <button id="button_receber_cancel" type="button" class="btn btn-secondary"
                                    data-dismiss="modal">Não
                            </button>
                            <a href="${linkTo[ExMovimentacaoController].aReceber()}?sigla=${docVO.mob.sigla}"
                               onclick="sigaSpinner.mostrar();"
                               class="btn btn-primary btn-acao" role="button" aria-pressed="true"
                               style="margin-left: .5rem;">Sim</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <button id="button_receber_ok" type="button" class="btn btn-primary siga-btn-receber-doc" data-placement="left"
                title="Receber"
                data-siga-modal-abrir="modalReceberDocumento">
            ...
            <c:when test="${podeExibirTodosOsVolumes }">
                <siga:siga-modal id="modalDeConfirmacaoArqCorrente" exibirRodape="true"
                    ...
                    <div class="modal-body">
                        Verifique se há necessidade de incluir o Termo de Encerramento para este documento. Deseja
                        continuar com o arquivamento?
                    </div>
                </siga:siga-modal>
            </c:when>
            <c:otherwise>
    ...
    <c:if test="${mob.isJuntado()}">
        ...
        <c:if test="${mob.mobilPrincipal.isSobrestado() && mob.mobilPrincipal.doc.isComposto()}">
            <siga:siga-modal id="modalDeAvisoDesentranhar" exibirRodape="true"
                             ...
                             descricaoBotaoFechaModalDoRodape="Ok">
                <div class="modal-body">
                    Não é possível fazer o desentranhamento porque o documento ao qual este está juntado encontra-se
                    sobrestado.
                </div>
            </siga:siga-modal>
            <script>
