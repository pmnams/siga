...
<c:if test="${mob.doc.podeReordenar()}">
    ...
    <script type="text/javascript">
        ...
        const iframehide = "no"
        const getFFVersion = navigator.userAgent.substring(
            navigator.userAgent.indexOf("Firefox")).split("/")[1]
        let FFextraHeight = parseFloat(getFFVersion) >= 0.1 ? 3 : 0
    ...
    <script type="text/javascript">
        ...
        function redimensionar() {
            if ("${f:resource('/sigaex.conversor.html.ext')}" === "br.gov.jfrj.itextpdf.MyPD4ML") {
                ...
                if ($('#radioHTML').hasClass('active') || document.getElementById('radioHTML').checked) {
                    if (document.getElementsByClassName("divDoc").length > 0) {
                        const divs = document.getElementsByClassName("divDoc");

                        for (let i = 0; i < divs.length; i++) {
                            document.getElementsByClassName("divDoc")[i].style.width = document.getElementById('painel').clientWidth - 10;
                            document.getElementsByClassName("divDoc")[i].style.padding = "20px";
                        }
                    } else {
                        setTimeout(function () {
                            redimensionar();
        ...
        function telaCheia(b) {
            ...
            if ($('#TelaCheia').hasClass('btn-secondary')) {
                ...
                const divDocRight = document.getElementById('right-col');
    ...
    <div class="container-fluid content" id="page">
                ...
                <c:forEach var="m" items="${docVO.mobs}" varStatus="loop">
                    <c:if test="${((((mob.id == m.mob.id))))}">
                        <h3 style="margin-bottom: 0;">
                                ${m.getDescricaoCompletaEMarcadoresEmHtml(mob.doc.cadastrante, lota)}
        ...
        <div class="row">
            <div class="col">
                <c:choose>
                    <c:when test="${mob.doc.eletronico}">
        ...
        <div class="row mt-3">
                            ...
                            <table class="text-size-6 table table-hover table-sm table-striped m-0 mov tabela-documentos">
                                ...
                                <c:forEach var="arqNumerado" items="${arqsNum}">
                                    ...
                                    <c:if test="${arqNumerado.arquivo.class.simpleName == 'ExMovimentacao'}">
                                        ...
                                        <c:set var="mov" value="${arqNumerado.arquivo}" scope="page"/>
                            ...
                            <c:if test="${mob.doc.podeReordenar() && podeExibirReordenacao && mob.doc.temOrdenacao()}">
                                <div class="menu-retornar-para-original">
                                    ...
                                    <form action="${pageContext.request.contextPath}/app/expediente/doc/reordenar"
                                        ...
                                        <input type="hidden" name="sigla" value="${sigla}"/>
                                        <label for="isVoltarParaOrdemOriginal" class="checkbox-oculto">isVoltarParaOrdemOriginal</label>
                                        <input type="checkbox" name="isVoltarParaOrdemOriginal"
                                               id="isVoltarParaOrdemOriginal" class="checkbox-oculto" checked/>
            ...
            <div id="right-col" class="col-sm-8 col-lg-9">
                <div id="paipainel" style="margin: 0; clear: both;overflow:hidden;">
                    <script>
                        function verificarMensagem(url) {
                            if (url.includes("file=")) {
                                if ((document.getElementById('errorMessage') != null &&
                                    ...
                                    if (document.getElementsByClassName("textLayer").length === 0) {
                                        setTimeout(function () {
                    ...
                    <iframe style="visibility: visible; margin: 0; padding: 0; min-height: 20em;" name="painel"
