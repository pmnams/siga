    ...
    <div class="container-fluid">
        ...
        <table class="table table-hover table-sm">
            ...
            <c:forEach items="${classificacaoVigente}" var="cla">
                ...
                <c:choose>
                    <c:when test="${cla.numVias eq 0}">
                        <c:set var="vias" value="1"/>
                    </c:when>
                    <c:otherwise>
                        <c:set var="vias" value="${cla.numVias}"/>
                    </c:otherwise>
                </c:choose>

                </tr>
                <td rowspan="${vias}" class="align-middle">
                    <a title="${cla.descricao}"
                       href="editar?codificacao=${cla.codificacao}&acao=editar_classificacao">
                            ${cla.codificacao}
                    </a>
                </td>
                <td rowspan="${vias}" class="align-middle text-left">
                        ${cla.descrClassificacao}
                </td>

                <c:choose>
                    <c:when test="${cla.numVias ne 0}">
