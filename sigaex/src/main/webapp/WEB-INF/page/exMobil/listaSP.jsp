...
<c:if test="${empty msgPesqErro and (empty tamanho or tamanho == 0)}">
    <div id="msgPesqNaoEncontrou" class="alert alert-warning alert-dismissible fade show" role="alert">A pesquisa não
        retornou resultados.
    </div>
</c:if>
        ...
        <c:when test="${visualizacao == 2 or visualizacao == 4}">
            ...
            <script>
                var data = [
                    ...
                    , "${campos.get(campo)}"
                :
                '<c:out value="${documento[0].getFormConfidencial(titular, lotaTitular)[campo]}" />'
                </c:forEach>
                </c:if>
                },
                </c:forEach>
                ...
                <table class="gt-table table table-sm table-hover">
                    ...
                    <c:if test="${currentPageNumber > 1}">
                        <button type="button" class="btn btn-primary btn-sm active mr-1 mb-3"
                                onclick="javascript:sigaSpinner.mostrar();sbmt(${(currentPageNumber - 2) * itemPagina});">
                            Anterior
                        </button>
                    </c:if>
                    <c:if test="${itemPagina lt tamanho}">
                        <button type="button" class="btn btn-primary btn-sm active mr-1 mb-3"
                                onclick="javascript:sigaSpinner.mostrar();sbmt(${currentPageNumber * itemPagina});">
                            Próxima
                        </button>
                    </c:if>
                    <c:if test="${currentPageNumber ne 0}">
                        <button type="button" class="btn btn-primary btn-sm active mr-1 mb-3"
                                onclick="javascript:sigaSpinner.mostrar();sbmt(${0});">Voltar para o início
                        </button>
                    </c:if>
                    </tbody>
                </table>
