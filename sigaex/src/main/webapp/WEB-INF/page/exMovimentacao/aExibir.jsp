...
<siga:pagina titulo="Documento" popup="${popup}"
	...
	<c:choose>
		<c:when test="${mov.exTipoMovimentacao.idTpMov == ANEXACAO}">
			<c:set var="msgScript" value="anexo" />
		</c:when>
		<c:when test="${mov.exTipoMovimentacao.idTpMov == CANCELAMENTO_JUNTADA}">
			<c:set var="msgScript" value="desentranhamento" />
		</c:when>
		<c:when test="${mov.exTipoMovimentacao.idTpMov == CANCELAMENTO_DE_MOVIMENTACAO}">
			...
			<form name="frm" action="exibir" theme="simple" method="post">
								...
								<table border="0" style="width: 100%;">
									<tr>
										<td><c:set var="exibemov" scope="request" value="" /> <c:set
												...
												<c:if test='${mov.exTipoMovimentacao.idTpMov == JUNTADA}'>
													<c:set var="exibemov" scope="request" value="juntada" />
												</c:if>
												<c:if test='${mov.exTipoMovimentacao.idTpMov == COPIA}'>
													<c:set var="exibemov" scope="request" value="copia" />
												</c:if>
												<c:if test='${mov.exTipoMovimentacao.idTpMov == REFERENCIA}'>
												...
												<c:if test="${(doc.idDoc == mov.exDocumento.idDoc)}">
													<c:if
														test='${(mov.exTipoMovimentacao.idTpMov == DESPACHO) || (mov.exTipoMovimentacao.idTpMov == DESPACHO_TRANSFERENCIA) || (mov.exTipoMovimentacao.idTpMov == DESPACHO_TRANSFERENCIA_EXTERNA)}'>
														<c:set var="exibemov" scope="request" value="despacho" />
													</c:if>
													<c:if test='${(mov.exTipoMovimentacao.idTpMov == CANCELAMENTO_JUNTADA)}'>
														<c:set var="exibemov" scope="request"
															value="desentranhamento" />
													</c:if>
													<c:if test='${(mov.exTipoMovimentacao.idTpMov == ENCERRAMENTO_DE_VOLUME)}'>
														<c:set var="exibemov" scope="request" value="encerramento" />
													</c:if>
													<c:if test='${(mov.exTipoMovimentacao.idTpMov == CANCELAMENTO_DE_MOVIMENTACAO)}'>
				...
				<div class="card-footer text-center">
					<div style="padding-left: 10; padding-top: 10px;">
						...
						<c:if test="${mov.exTipoMovimentacao.idTpMov != ANEXACAO}">
							<button type="button" class="btn btn-info"
		    	...
		    	<div class="card">
					<div class="card-body">
						...
						<tags:assinatura_botoes assinar="true"
							autenticar="${mov.exTipoMovimentacao.idTpMov == ANEXACAO}"
