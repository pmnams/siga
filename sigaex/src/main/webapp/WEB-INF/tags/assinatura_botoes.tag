    ...
    <div class="row">
        <div class="col-auto my-auto">
            ...
            <c:if test="${not empty voltar and voltar}">
                <input type="button" value="Voltar"
                       onclick="${(empty linkVoltar) ? 'javascript:history.back();' : 'javascript:window.location.href=\''.concat(linkVoltar).concat('\';')}"
        ...
        <c:if test="${assinarCertDigital or assinarComSenha or assinarComSenhaPin or autenticarComSenha or autenticarComSenhaPin or autenticar}">
            <div class="col-auto my-auto mr-5">
                <h5>Formas
                    de ${(not empty assinar and assinar)? 'assinatura': (not empty autenticar and autenticar) ? 'autenticação':''}:</h5>

                <c:set var="SenhaChecked" value="${assinarComSenhaChecado || autenticarComSenhaChecado}"/>
                <c:set var="PinChecked"
                       value="${(not empty identidadeCadastrante.pinIdentidade) and (assinarComSenhaPinChecado || autenticarComSenhaPinChecado)}"/>
                    ...
                    <div class="custom-control custom-radio">
                        ...
                        <c:if test="${empty identidadeCadastrante.pinIdentidade}">
                            <small class="text-muted"> - Clique <strong><a
                                    href='/siga/app/pin/cadastro'>aqui</a></strong> saber mais e definir seu
                                PIN.</small>
                        </c:if>
                    </div>
                </c:if>
                ...
                <c:if test="${not empty exibirNoProtocoloAtivo}">
                    <div class="custom-control custom-checkbox ">
                        <input class="form-check-input" type="checkbox" name="ad_exibirNoProtocolo_0"
                               id="ad_exibirNoProtocolo_0" onchange="confirmaExibirNoProtocolo(this)"
                               <c:if test="${exibirNoProtocoloAtivo}">checked</c:if>
                               <c:if test="${exibirNoProtocoloFixo}">disabled</c:if> /> <label
                            class="form-check-label" for="ad_exibirNoProtocolo_0">Disponibilizar no Acompanhamento do
                        Protocolo</label>
                    </div>
