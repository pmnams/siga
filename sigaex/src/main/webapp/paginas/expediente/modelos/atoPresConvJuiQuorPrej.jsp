...
<mod:modelo urlBase="/paginas/expediente/modelos/ato_presidencia.jsp">
	<mod:entrevista>
		<mod:grupo titulo=" ">
			<mod:grupo>
				<mod:selecao var="presid" titulo="VOCATIVO"
					opcoes="A PRESIDENTE;O PRESIDENTE" reler="sim" />
			</mod:grupo>
		</mod:grupo>
		...
		<mod:grupo titulo="Juiz Federal Convocado">
			<mod:grupo>
				...
				<mod:selecao titulo="Com prejuízo" var="prej2" opcoes="sim;não"
					reler="não" />
			</mod:grupo>
			<mod:grupo>
		...
		<mod:grupo titulo="Desembargador Federal Substituído">
			<mod:grupo>
				...
				<mod:selecao titulo="em virtude" var="motiv"
					opcoes="de licença médica;das férias regulamentares"
					reler="não" />
			</mod:grupo>
			<mod:grupo>
	...
	<mod:documento>
		<c:set var="pessoa_titular"
			value="${f:pessoa(requestScope['titular_pessoaSel.id'])}" />
		<c:set var="pessoa_convoc"
			value="${f:pessoa(requestScope['convocado_pessoaSel.id'])}" />

		<mod:valor var="texto_ato">
			<br />
			<br />
			<p style="TEXT-INDENT: 2cm" align="justify">
				${presid} DO TRIBUNAL REGIONAL FEDERAL DA 2ª REGIÃO, no uso de
				suas atribuições, RESOLVE: CONVOCAR
				<c:choose>
					<c:when test="${ gen1 == 'M'}">
				...
				da ${pessoa_convoc.lotacao.descricao},
				<c:choose>
				...
				prejuizo de sua jurisdição, compor o quorum deste Tribunal, no
				período de ${dtini} a ${dtfin}, em virtude ${motiv}
				<c:choose>
				...
				${ pessoa_titular.nomePessoa}, nos termos do artigo 48, inciso I, do
				Regimento Interno desta Corte c/c artigo 1º, inciso I, da
				Resolução nº 51/2009, do Conselho da Justiça Federal.
