...
public class RelatorioDocumentosSubordinados extends RelatorioTemplate {
    ...
    public AbstractRelatorioBaseBuilder configurarRelatorio()
        ...
        this.addColuna("Responsável", 40, RelatorioRapido.ESQUERDA, false);
        this.addColuna("Situação", 30, RelatorioRapido.ESQUERDA, false);
        this.addColuna("Duração", 15, RelatorioRapido.CENTRO, false);
        return this;
    ...
    public Collection processarDados() throws Exception {
        ...
        Query qryMarcas = ContextoPersistencia.em()
                .createQuery(
                        "select " + "	marca.dpLotacaoIni.nomeLotacao, "
                                ...
                                + ","
                                + "	pes.nomePessoa,"
                                + "	marca.cpMarcador.descrMarcador, marca.dtIniMarca "
                                + "from ExMarca marca "
        ...
        for (Object[] array : lista) {
            ...
            if (Ex.getInstance().getBL().exibirQuemTemAcessoDocumentosLimitados(
                ...
                long days = LocalDateTime.now().until(((Timestamp) array[23]).toLocalDateTime(), ChronoUnit.DAYS) * -1;
                if (days > 0)
                    listaFinal.add(String.valueOf(days));
                else
                    listaFinal.add("-");
            }
        }
