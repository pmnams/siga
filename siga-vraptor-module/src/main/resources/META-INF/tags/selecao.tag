    ...
    var modalsimulawindow${propriedade} = function (url) {
        try {
            ...
            var senhaDialog${propriedade} = $(
                ...
                + '	        <button type="button" id="btnsenhaDialog${propriedade}" class="close  p-3  m-1  siga-modal__btn-close" aria-label="Close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>'
                + '    </div>'
    ...
    <c:if test="${ocultardescricao != 'sim'}">
        <div class="input-group-append ml-2" style="width: 60%;">
            <span class="form-control" style="overflow: hidden; white-space:nowrap; text-overflow:ellipsis;"
                  id="${spanName}SelSpan"><c:out value="${requestScope[propriedadeTipoSel].descricao}"
