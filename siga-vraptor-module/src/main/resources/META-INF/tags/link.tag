...
<c:if test="${(atalho == true) and fn:contains(title, '_')}">
    ...
    <c:set var="title">${fn:substring(title, 0, ious)}
        <u>${fn:substring(title, ious+1, ious+2)}</u>${fn:substring(title, ious+2, -1)}</c:set>
</c:if>
<c:set var="linkId"
...
<c:if test="${(atalho == true) and fn:contains(title, '_')}">
    <c:set var="ious" value="${fn:indexOf(title, '_')}"/>
    <c:set var="title">${fn:substring(title, 0, ious)}
        <u>${fn:substring(title, ious+1, ious+1)}</u>${fn:substring(title, ious+2, -1)}</c:set>
</c:if>
...
<c:if test="${exibirExplicacao and (not empty explicacao or not empty descr)}">
    <c:set var="tooltip">
        ...
        <c:if test="${not empty descr}">
            <hr/>
            <p style='margin-bottom: 0;'>${descr}</p>
        </c:if>
        <c:if test="${not empty explicacao}">
            <hr/>
            <p style='font-size: 70%; color: gray; margin-bottom: 0;'>${explicacao}</p>
        </c:if>
    </c:set>
</c:if>
<c:choose>
            ...
            <c:choose>
                ...
                <c:when test="${not empty ajax and ajax != false}">
					<span id="spanAjax_${idAjax}"> <a id="${linkId}"
                                                      ...
                                                      data-toggle="tooltip" data-html="true" data-delay="2000"
                                                      title="${tooltip}">${img}${title}</a>
					</span>
                </c:when>
                <c:otherwise>
                    <c:choose>
